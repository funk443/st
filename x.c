...
int evcol(XEvent *e) {
  int x = e->xbutton.x - borderpx_left;
  LIMIT(x, 0, win.tw - 1);
  return x / win.cw;
}

int evrow(XEvent *e) {
  int y = e->xbutton.y - borderpx_top;
  LIMIT(y, 0, win.th - 1);
  return y / win.ch;
}
...
void cresize(int width, int height) {
  ...
  col = (win.w - borderpx_left - borderpx_right) / win.cw;
  row = (win.h - borderpx_top - borderpx_bottom) / win.ch;
  col = MAX(1, col);
  row = MAX(1, row);
...
void xhints(void) {
  ...
  sizeh->base_height = borderpx_top + borderpx_bottom;
  sizeh->base_width = borderpx_left + borderpx_right;
  sizeh->min_height = win.ch + sizeh->base_height;
  sizeh->min_width = win.cw + sizeh->base_width;
  if (xw.isfixed) {
...
void xinit(int cols, int rows) {
  ...
  win.w = borderpx_left + borderpx_right + cols * win.cw;
  win.h = borderpx_top + borderpx_bottom + rows * win.ch;
  if (xw.gm & XNegative)
...
int xmakeglyphfontspecs(XftGlyphFontSpec *specs, const Glyph *glyphs, int len,
                        int x, int y) {
  float winx = borderpx_left + x * win.cw;
  float winy = borderpx_top + y * win.ch;
  float xp, yp;
...
void xdrawglyphfontspecs(const XftGlyphFontSpec *specs, Glyph base, int len,
  ...
  int winx = borderpx_left + x * win.cw;
  int winy = borderpx_top + y * win.ch;
  ...
  if (x == 0) {
    xclear(0, (y == 0) ? 0 : winy, borderpx_left,
           winy + win.ch +
               ((winy + win.ch >= borderpx_top + win.th) ? win.h : 0));
  }
  if (winx + width >= borderpx_left + win.tw) {
    xclear(
        winx + width, (y == 0) ? 0 : winy, win.w,
        ((winy + win.ch >= borderpx_left + win.th) ? win.h : (winy + win.ch)));
  }
  if (y == 0)
    xclear(winx, 0, winx + width, borderpx_top);
  if (winy + win.ch >= borderpx_top + win.th)
...
void xdrawcursor(int cx, int cy, Glyph g, int ox, int oy, Glyph og) {
  ...
  if (IS_SET(MODE_FOCUSED)) {
    ...
    case 4: /* Steady Underline */
      XftDrawRect(xw.draw, &drawcol, borderpx_left + cx * win.cw,
                  borderpx_top + (cy + 1) * win.ch - cursorthickness, win.cw,
                  cursorthickness);
      break;
    case 5: /* Blinking bar */
    case 6: /* Steady bar */
      XftDrawRect(xw.draw, &drawcol, borderpx_left + cx * win.cw,
                  borderpx_top + cy * win.ch, cursorthickness, win.ch);
      break;
    }
  } else {
    XftDrawRect(xw.draw, &drawcol, borderpx_left + cx * win.cw,
                borderpx_top + cy * win.ch, win.cw - 1, 1);
    XftDrawRect(xw.draw, &drawcol, borderpx_left + cx * win.cw,
                borderpx_top + cy * win.ch, 1, win.ch - 1);
    XftDrawRect(xw.draw, &drawcol, borderpx_left + (cx + 1) * win.cw - 1,
                borderpx_top + cy * win.ch, 1, win.ch - 1);
    XftDrawRect(xw.draw, &drawcol, borderpx_left + cx * win.cw,
                borderpx_top + (cy + 1) * win.ch - 1, win.cw, 1);
  }
}
...
void xximspot(int x, int y) {
  ...
  xw.ime.spot.x = borderpx_left + x * win.cw;
  xw.ime.spot.y = borderpx_top + (y + 1) * win.ch;
